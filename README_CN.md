...
- 会话记录支持逐条删除：用户消息和 AI 回复可点击删除，删除后会同步当前对话历史、会话缓存和恢复视图。
- 聊天 UI 的长文本、AI 回复、代码块以及 API 错误/状态文本会随窗口宽度重新换行，避免横向裁切。
- 发送后的图片缩略图和 `capture_viewport` 截图会保留在会话时间线中，并可点击打开完整预览窗口。
- 支持视觉模型的 Agent 运行会在场景修改阶段结束后自动附加视口检查截图，让模型按阶段验证 Houdini 结果，而不是每次编辑都截图。
- 工作流经验审阅支持一次提取多条候选经验、显示/隐藏已拒绝项、删除已拒绝候选，并将精选语义/策略经验导出为 Markdown。
...
- **当前更新** — **工作流经验审阅与晋升流程**：新增始终开启的工作流经验候选提取与四栏审阅台（候选 / 晋升 / 稍后 / 拒绝），右侧详情展示去噪总结、证据、置信度和晋升结果。经验沉淀改为从上下文中提取结论、适用场景、验证方式和有效证据，过滤 `<think>`、Todo 续接提示和工具流水账。修复审阅窗口右上角关闭按钮无效的问题，并更新长期记忆文档。
